...
const Subcategory = ({ Subcategories, CategoryParams, CategoryName }) => {
  return (
    <div>
      <div>
            ...
            <div className=" w-[50%]">
              <ul className="">
                {Subcategories?.map((item, index) => {
                  return (
                    <Link
                      href={`/Location/?category=${CategoryName}&categoryParams=${CategoryParams}&?subcat=${item?.name}&subcatid=${item?.id}`}
                    >
                      <li
                        key={index}
